...
#if !defined(Adc_CbmiRangeProvider_hpp)
#define Adc_CbmiRangeProvider_hpp
...
class CbmiRangeProvider : public ThreadSafeProvider
{
    public:
        typedef ThreadSafeProvider Base;
        CbmiRangeProvider();
        virtual ~CbmiRangeProvider();
    ...
    private:
        void ClearBufferVariables() {
            m_commit_cbmi_low = 0xFFFF;
            m_commit_cbmi_high = 0xFFFF;
        }

        SimAuxHandler * m_aux_handler;
        UINT16          m_commit_cbmi_low;
        UINT16          m_commit_cbmi_high;
};
