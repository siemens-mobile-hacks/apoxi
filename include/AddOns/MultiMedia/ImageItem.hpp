/*  ------------------------------------------------------------------------
    Copyright (C) COMNEON electronic technology GmbH&  Co. OHG.
    All rights reserved.
    ------------------------------------------------------------------------
    This document contains proprietary information belonging to COMNEON.
    Passing on and copying of this document, use and communication of its
    contents is not permitted without prior written authorisation.
    ------------------------------------------------------------------------ */

#if !defined(Apoxi_ImageItem_hpp)
#define Apoxi_ImageItem_hpp

#include <Gui/ImageObtainer.hpp>
#include <AddOns/MultiMedia/BoundedItem.hpp>

class ImageItem : public BoundedItem {
    typedef BoundedItem Base;

    public:
        enum {
            ID = 4
        };

        explicit ImageItem(const ImageObtainer& obtainer);

        virtual ~ImageItem();

        virtual INT GetId() const;

        virtual XYDIM GetWidth() const;

        virtual XYDIM GetHeight() const;

        virtual void Draw(DeviceContext* dc, const Rect& bounds) const;

        const ImageObtainer& GetImageObtainer() const { return m_obtainer; }

        const Image* GetImage() const { return m_obtainer.GetImage(); }

        virtual AtomItem* Clone() const;

        virtual BOOLEAN IsInstanceOf(INT id) const;
    
    private:
        ImageObtainer m_obtainer;
};

#endif  // Apoxi_ImageItem_hpp

