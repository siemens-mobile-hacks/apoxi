...
#if !defined(Apoxi_VirtualDisplay_hpp)
#define Apoxi_VirtualDisplay_hpp
...
class ScanlineAccessor {
    public:
        ...
        {
            m_display->SetColor(Color(red, green, blue));
            m_display->SetPixel(m_x + m_xpos, m_y);
            m_xpos++;
            m_count--;
        }
        
        void Draw(ColorValue color)
        {
            m_display->SetColorValue(color);
            m_display->SetPixel(m_x + m_xpos, m_y);
            m_xpos++;
            m_count--;
        }
...
//  The optimized implementation for 16bpp displays is left in here for reference
//
//  public:
//      ScanlineAccessor(VirtualDisplay* display, XYDIM x, XYDIM y, XYDIM width) :
...
//
//  private:
//      /// pointer to the current line onscreen
//      UINT16 GDI_HUGE* m_data;
//      /// used to determine if there are pixels left to draw
//      XYDIM m_count;

};

#endif  // Apoxi_VirtualDisplay_hpp
