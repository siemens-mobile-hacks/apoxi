...
class EmzMpeg4EncoderThread : public Thread {
    ...
    private:
        ...
        CodecManager::ResultType Start(
                            ...
                            EmzMpeg4Encoder* video_enc);

        void Stop();

        CodecManager::ResultType GetConfigStream(   UINT8 HUGE* out_buf,
                                                    UINT32& out_buf_len);

        CodecManager::ResultType SetParameters( const VideoEncoder::ParameterType* types_array,
                                                const UINT32* values_array,
                                                UINT8 array_count);

        CodecManager::ResultType GetParameters( const VideoEncoder::ParameterType* types_array,
                                                UINT32* values_array,
                                                UINT8 array_count);
