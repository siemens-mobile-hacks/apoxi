...
class EmzH264DecoderThread : public Thread {
    ...
    private:
        ...
        virtual CodecDevice::ResultType Start( EmzH264Decoder* parent);

        virtual void Stop();

        CodecDevice::ResultType SetParameters(  const VideoDecoder::ParameterType* types_array,
                                                const UINT32* values_array,
                                                UINT8 array_count);

        CodecDevice::ResultType GetParameters(  const VideoDecoder::ParameterType* types_array,
                                                UINT32* values_array,
                                                UINT8 array_count);
        ...
        CodecDevice::ResultType SetConfig( const UINT8* config_stream,
                                                    UINT32& config_len,
                                                    const CodecDevice::VideoDecoderParameters& params,
                                                    CodecDevice::VideoStreamInfo& stream_info);
